...
void ADC1_SS3__vIRQVectorHandler(void)
{
    ...
    {
        ADC1_ISC_R = ((uint32_t) ADC_enSEQ_SOURCE_SAMPLE << (uint32_t) ADC_enSEQ_3);
    ...
    if(u32Reg & ((uint32_t) ADC_enSEQ_SOURCE_COMP << (uint32_t) ADC_enSEQ_3))
     {
         ADC1_ISC_R = ((uint32_t) ADC_enSEQ_SOURCE_COMP << (uint32_t) ADC_enSEQ_3);
         if(u32RegCompSelect & ((uint32_t) 0x1UL << (ADC_en_MUX_0 * 0x4UL)))
         {
             u32RegCompMux = ADC1_SSDC3_R >> (ADC_en_MUX_0 * 0x4UL);
             u32RegCompMux &= 0xFUL;
