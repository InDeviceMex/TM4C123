...
void ADC0_SS0__vIRQVectorHandler(void)
{
    ...
    if(0UL != u32RegDMAEn)
    {
        ...
        if(0UL != u32RegDMAOccur)
        {
            ...
            if(0UL == u32RegPeriph)
            {
                ...
                if(DMA_CHMAP1_R_CH14SEL_ADC0_SS0 == u32RegDMASource)
                {
    ...
    {
        ADC0_ISC_R = ((uint32_t) ADC_enSEQ_SOURCE_SAMPLE << (uint32_t) ADC_enSEQ_0);
    ...
    {
        ADC0_ISC_R = ((uint32_t) ADC_enSEQ_SOURCE_COMP << (uint32_t) ADC_enSEQ_0);
        for(u32Pos = 0UL; u32Pos <= (uint32_t) ADC_en_MUX_7; u32Pos++)
        {
            if(u32RegCompSelect & ((uint32_t) 0x1UL << (u32Pos * 0x4UL)))
            {
                u32RegCompMux = ADC0_SSDC0_R >> (u32Pos * 0x4UL);
                u32RegCompMux &= 0xFUL;
                if(u32RegCompInterrupt & ((uint32_t) 1UL << u32RegCompMux))
                {
                    ADC0_DCISC_R = ((uint32_t) 1UL << u32RegCompMux);
                    ADC_COMP__vIRQSourceHandler[(uint32_t) ADC_enMODULE_0][(uint32_t) ADC_enSEQ_0][u32RegCompMux]();
                }
            }
        }
    }
