...
{
    ADC__vSetSequencerGeneric((uint32_t) enModule, ADC_ACTSS_OFFSET, (uint32_t) enSequence, (uint32_t) enSeqEnable);
}

ADC_nSEQ_ENABLE ADC__enGetSequencerEnable(ADC_nMODULE enModule, ADC_nSEQMASK enSequence)
{
    return (ADC_nSEQ_ENABLE) (ADC__u32GetSequencerGeneric((uint32_t) enModule, ADC_ACTSS_OFFSET, (uint32_t) enSequence));
}
