...
HIB_nSTATUS HIB__enGetCounter(uint32_t* pu32Counter, uint32_t *pu32SubCounter)
{
    ...
    {
        enReturn = HIB__enReadRegister(HIB_RTCC_OFFSET, pu32Counter, HIB_RTCC_RTCC_MASK, HIB_RTCC_R_RTCC_BIT);

        if(HIB_enSTATUS_OK == enReturn)
        {
            enReturn = HIB_enSTATUS_ERROR;
            HIB__enReadRegister(HIB_RTCSS_OFFSET, pu32SubCounter, HIB_RTCSS_RTCSSC_MASK, HIB_RTCSS_R_RTCSSC_BIT);
            HIB__enReadRegister(HIB_RTCC_OFFSET, &u32SecondRead, HIB_RTCC_RTCC_MASK, HIB_RTCC_R_RTCC_BIT);
