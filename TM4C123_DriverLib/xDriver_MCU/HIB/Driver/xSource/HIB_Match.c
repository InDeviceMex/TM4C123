...
HIB_nSTATUS HIB__enGetMatch(uint32_t* pu32Match, uint32_t *pu32SubMatch)
{
    ...
    {
        enReturn = HIB__enReadRegister(HIB_RTCM0_OFFSET, pu32Match, HIB_RTCM0_RTCM0_MASK, HIB_RTCM0_R_RTCM0_BIT);
        if(HIB_enSTATUS_OK == enReturn)
        {
            enReturn = HIB__enReadRegister(HIB_RTCSS_OFFSET, pu32SubMatch, HIB_RTCSS_RTCSSM_MASK, HIB_RTCSS_R_RTCSSM_BIT);
        }
    }
    return enReturn;
}
...
HIB_nSTATUS HIB__enSetMatch(uint32_t u32Match, uint32_t u32SubMatch)
{
    ...
    enReturn = HIB__enWriteRegister(HIB_RTCM0_OFFSET, u32Match, HIB_RTCM0_RTCM0_MASK, HIB_RTCM0_R_RTCM0_BIT);
    if(HIB_enSTATUS_OK == enReturn)
    {
        enReturn = HIB__enWriteRegister(HIB_RTCSS_OFFSET, u32SubMatch, HIB_RTCSS_RTCSSM_MASK, HIB_RTCSS_R_RTCSSM_BIT);
    }
    return enReturn;
}
