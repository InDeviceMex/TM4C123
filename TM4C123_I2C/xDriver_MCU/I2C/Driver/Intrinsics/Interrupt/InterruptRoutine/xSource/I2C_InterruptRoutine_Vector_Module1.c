...
void I2C1__vIRQVectorHandler(void)
{
    ...
    {
        I2C1_I2CMICR_R = (uint32_t) I2C_enINT_SOURCE_MASTER_MASTER;
        I2C_Master__vIRQSourceHandler[(uint32_t) I2C_enMODULE_1][(uint32_t) I2C_enINTERRUPT_MASTER_MASTER]();
    }
    if((uint32_t) I2C_enINT_SOURCE_MASTER_CLOCK_TIMEOUT & u32RegMaster)
    {
        I2C_Master__vIRQSourceHandler[(uint32_t) I2C_enMODULE_1][(uint32_t) I2C_enINTERRUPT_MASTER_CLOCK_TIMEOUT]();
        I2C1_I2CMICR_R = (uint32_t) I2C_enINT_SOURCE_MASTER_CLOCK_TIMEOUT;
    }

    if((uint32_t) I2C_enINT_SOURCE_SLAVE_DATA & u32RegSlave)
    {
        I2C1_I2CSICR_R = (uint32_t) I2C_enINT_SOURCE_SLAVE_DATA;
        I2C_Slave__vIRQSourceHandler[(uint32_t) I2C_enMODULE_1][(uint32_t) I2C_enINTERRUPT_SLAVE_DATA]();
    }
    if((uint32_t) I2C_enINT_SOURCE_SLAVE_START & u32RegSlave)
    {
        I2C1_I2CSICR_R = (uint32_t) I2C_enINT_SOURCE_SLAVE_START;
        I2C_Slave__vIRQSourceHandler[(uint32_t) I2C_enMODULE_1][(uint32_t) I2C_enINTERRUPT_SLAVE_START]();
    }
    if((uint32_t) I2C_enINT_SOURCE_SLAVE_STOP & u32RegSlave)
    {
        I2C1_I2CSICR_R = (uint32_t) I2C_enINT_SOURCE_SLAVE_STOP;
        I2C_Slave__vIRQSourceHandler[(uint32_t) I2C_enMODULE_1][(uint32_t) I2C_enINTERRUPT_SLAVE_STOP]();
    }
}
