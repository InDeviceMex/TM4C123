...
#include "HIB.h"
#include "GPIO.h"
volatile HIB_nREADY HIB_enStatus =HIB_enSTATUS_BUSY;
HIB_nREADY HIB_enCount=HIB_enSTATUS_BUSY;

void HIB_vISR(void);

HIB_nSTATUS HIB__enInit(uint32_t u32Match, uint32_t u32SubMatch)
{
    volatile HIB_nSTATUS enReturn = HIB_enSTATUS_OK;
    ...
    while(HIB_enSTATUS_BUSY == HIB_enStatus)
    {
        u32TimeOut--;
        if(u32TimeOut==0)
        {
    ...
    if(HIB_enSTATUS_OK == enReturn)
    {
        enReturn= HIB__enClearInterrupt(HIB_enINT_ALL);
    }

    if(HIB_enSTATUS_OK == enReturn)
    {
        enReturn= HIB__enEnInterrupt(HIB_enINT_RTCALT);
    }
