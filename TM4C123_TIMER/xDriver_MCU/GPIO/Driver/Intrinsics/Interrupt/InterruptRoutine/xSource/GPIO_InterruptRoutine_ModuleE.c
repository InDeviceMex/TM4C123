    ...
    volatile uint32_t u32Reg=0;
     ...
     if(GPIO_enAPB ==u32RegBUS)
     {
         ...
         if(u32Reg & GPIO_enPIN0)
         {
             ...
             GPIO__vIRQSourceHandler[(uint32_t)GPIO_enPORTE][(uint32_t)GPIO_enPIN_NUMBER0]();
         }
         if(u32Reg & GPIO_enPIN1)
         {
             GPIOE_APB_GPIOICR_R=GPIO_enPIN1;
             GPIO__vIRQSourceHandler[(uint32_t)GPIO_enPORTE][(uint32_t)GPIO_enPIN_NUMBER1]();
         }
         if(u32Reg & GPIO_enPIN2)
         {
             GPIOE_APB_GPIOICR_R=GPIO_enPIN2;
             GPIO__vIRQSourceHandler[(uint32_t)GPIO_enPORTE][(uint32_t)GPIO_enPIN_NUMBER2]();
         }
         if(u32Reg & GPIO_enPIN3)
         {
             GPIOE_APB_GPIOICR_R=GPIO_enPIN3;
             GPIO__vIRQSourceHandler[(uint32_t)GPIO_enPORTE][(uint32_t)GPIO_enPIN_NUMBER3]();
         }
         if(u32Reg & GPIO_enPIN4)
         {
             GPIOE_APB_GPIOICR_R=GPIO_enPIN4;
             GPIO__vIRQSourceHandler[(uint32_t)GPIO_enPORTE][(uint32_t)GPIO_enPIN_NUMBER4]();
         }
         if(u32Reg & GPIO_enPIN5)
         {
             GPIOE_APB_GPIOICR_R=GPIO_enPIN5;
             GPIO__vIRQSourceHandler[(uint32_t)GPIO_enPORTE][(uint32_t)GPIO_enPIN_NUMBER5]();
         }
         if(u32Reg & GPIO_enPIN6)
         {
             GPIOE_APB_GPIOICR_R=GPIO_enPIN6;
             GPIO__vIRQSourceHandler[(uint32_t)GPIO_enPORTE][(uint32_t)GPIO_enPIN_NUMBER6]();
         }
         if(u32Reg & GPIO_enPIN7)
         {
             GPIOE_APB_GPIOICR_R=GPIO_enPIN7;
             GPIO__vIRQSourceHandler[(uint32_t)GPIO_enPORTE][(uint32_t)GPIO_enPIN_NUMBER7]();
         }
     }
     else
     {
