...
void DMA_SW__vIRQVectorHandler(void)
{
    ...
    uint32_t u32Assign = 0u;
    uint32_t u32Perihepral = DMA_DMAREQMASKSET_R;
    ...
    for( u32Channel=0u; u32Channel<=(uint32_t)DMA_enCH_MODULE_MAX; u32Channel++)
    {
        ...
        {
            u32Assign= *DMA_ChannelAssig[u32Channel];
            u32Assign>>=DMA_ChannelAssigBit[u32Channel];
            u32Assign&=0xFu;
            if(((uint32_t) DMA_enCH_ENCODER_4 == u32Assign) || (0u == (u32Perihepral&u32Pos) ) )
            {
