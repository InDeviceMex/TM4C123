...
{
    DMA_CH__vSetPrimaryControlGeneric( enChannel, (uint32_t) enChannelSourceSize, DMACH_DMACHCTL_SRCSIZE_MASK, DMACH_DMACHCTL_R_SRCSIZE_BIT);
}

void DMA_CH__vSetAlternateSourceSize(DMA_nCH_MODULE enChannel, DMA_nCH_SRC_SIZE enChannelSourceSize)
{
    DMA_CH__vSetAlternateControlGeneric( enChannel, (uint32_t) enChannelSourceSize, DMAALTCH_DMACHCTL_SRCSIZE_MASK, DMAALTCH_DMACHCTL_R_SRCSIZE_BIT);
}

void DMA_CH__vSetSourceSize(DMA_nCH_MODULE enChannel, DMA_nCH_CTL enChannelStructure, DMA_nCH_SRC_SIZE enChannelSourceSize)
{
    DMA_CH__vSetControlGeneric( enChannel, enChannelStructure, (uint32_t) enChannelSourceSize, DMACH_DMACHCTL_SRCSIZE_MASK, DMACH_DMACHCTL_R_SRCSIZE_BIT);
}

DMA_nCH_SRC_SIZE DMA_CH__enGetPrimarySourceSize(DMA_nCH_MODULE enChannel)
{
    DMA_nCH_SRC_SIZE enReg = DMA_enCH_SRC_SIZE_UNDEF;
    enReg = (DMA_nCH_SRC_SIZE) DMA_CH__u32GetPrimaryControlGeneric( enChannel, DMACH_DMACHCTL_SRCSIZE_MASK, DMACH_DMACHCTL_R_SRCSIZE_BIT);
    return enReg;
}
...
{
    DMA_nCH_SRC_SIZE enReg = DMA_enCH_SRC_SIZE_UNDEF;
    enReg = (DMA_nCH_SRC_SIZE) DMA_CH__u32GetControlGeneric( enChannel, enChannelStructure, DMACH_DMACHCTL_SRCSIZE_MASK, DMACH_DMACHCTL_R_SRCSIZE_BIT);
    return enReg;
}
