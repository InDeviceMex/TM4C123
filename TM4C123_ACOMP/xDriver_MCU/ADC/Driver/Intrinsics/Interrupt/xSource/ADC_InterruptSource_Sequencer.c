...
void ADC__vEnSeqInterruptSource(ADC_nMODULE enModule, ADC_nSEQMASK enSequence, ADC_nINT_SOURCE enSourceInt)
{
    ...
    {
        ADC__vWriteRegister(enModule , ADC_ADCIM_OFFSET, (uint32_t) enSequence, (uint32_t) ADC_enSEQMASK_MAX, u32BitPos);
    }
    else
    {
        ADC__vWriteRegister(enModule , ADC_ADCIM_OFFSET, (uint32_t) enSequence, (uint32_t) enSequence, u32BitPos);
    }
}
...
void ADC__vDisSeqInterruptSource(ADC_nMODULE enModule, ADC_nSEQMASK enSequence, ADC_nINT_SOURCE enSourceInt)
{
    ...
    if(ADC_enINT_SOURCE_COMP == enSourceInt)
    {
        ADC__vWriteRegister(enModule , ADC_ADCIM_OFFSET, 0UL, (uint32_t) ADC_enSEQMASK_MAX, u32BitPos);
    }
    else
    {
        ADC__vWriteRegister(enModule , ADC_ADCIM_OFFSET, 0UL, (uint32_t) enSequence, u32BitPos);
    }
}
