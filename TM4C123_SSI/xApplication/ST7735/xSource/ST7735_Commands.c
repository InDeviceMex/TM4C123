...
void ST7735__vCommandList(const uint8_t * pu8AddressArg)
{
    ...
    for(u8NumCommandsPos = u8NumCommands; 0U != u8NumCommandsPos; u8NumCommandsPos--)
    {
        ...
        ST7735__u32WriteCommand(u8AddressValue);
        pu8AddressArg += 1UL;
        u8NumArguments  = *(pu8AddressArg);
        pu8AddressArg += 1UL;
        u16Delay = u8NumArguments & (uint8_t) ST7735_enCOMMAND_DELAY;          /*   If hibit set, delay follows args*/
        u8NumArguments &= ~ (uint8_t)  ST7735_enCOMMAND_DELAY;                   /*   Mask out delay bit*/

        for(u8NumArgumentsPos = u8NumArguments; 0U != u8NumArgumentsPos; u8NumArgumentsPos--)
        {
            u8AddressValue = *(pu8AddressArg);
            ST7735__u32WriteData(u8AddressValue);
            pu8AddressArg += 1UL;
        }

        if(0U != u16Delay)
        {
            u16Delay = *(pu8AddressArg);             /* Read post-command delay time (u16Delay)*/
            pu8AddressArg += 1UL;
            if(255U == u16Delay)
            {
                u16Delay = 500U;     /* If 255, delay for 500 u16Delay*/
            }
            ST7735__vDelay1ms(u16Delay);
        }
    }
}
