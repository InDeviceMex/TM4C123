...
int32_t FLASH_Write(int32_t data, int32_t address)
{
    ...
    if((*(uint32_t*)address)==0xFFFFFFFF)
    {
        ...
        while(FLASH_FMC_WRITE_BITBANDING)
        {
            if(timeout==0)
                return 0;
            timeout--;
...
int32_t FLASH_ErasePage(int32_t address)
{
    ...
    while(FLASH_FMC_ERASE_BITBANDING)
    {
        if(timeout==0)
            return 0;
        timeout--;
...
int32_t FLASH_Erase(void)
{
    ...
    while(FLASH_FMC_MERASE_BITBANDING)
    {
        if(timeout==0)
            return 0;
        timeout--;
...
int32_t FLASH_WriteBuf(int32_t* data,int32_t* pos,int32_t cant, int32_t address)
{
    ...
    while(FLASH_FMC2_WRBUF_BITBANDING)
    {
        if(timeout==0)
            return 0;
        timeout--;
